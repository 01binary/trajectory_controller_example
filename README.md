...
The joints to be controlled can be listed under the `joints` setting.

The `default` setting is used to indicate a default controller that will be chosen to control this set of joints. This is useful when additional controllers are defined for the joints:

* One such scenario is using the [Motion Planning RViz Plugin with a joystick](https://ros-planning.github.io/moveit_tutorials/doc/joystick_control_teleoperation/joystick_control_teleoperation_tutorial.html?highlight=joystick). In this case, one of the controllers supported by the joystick interface (such as  `JointGroupVelocityController` or `JointGroupPositionController`) will also be configured in `ros_controllers.yaml` for the same set of joints.
* Another scenario is configuring the robot for use with [MoveIt Servo](https://ros-planning.github.io/moveit_tutorials/doc/realtime_servo/realtime_servo_tutorial.html) which bypasses the normal motion planning pipeline and lets you control the robot by using a joystick or a [SpaceMouse](https://3dconnexion.com/us/).
